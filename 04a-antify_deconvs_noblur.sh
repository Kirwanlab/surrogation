...
if [ ! -f ${deconvName}_ANTS_resampled+tlrc.HEAD ]; then
	#convert from nifti to afni if needed
	if [ ! -f ${deconvName}+orig.HEAD ]; then
		3dcopy ${deconvName}.nii.gz ${deconvName}+orig
	fi
	
	#ANTify the deconvolution data
	${studyDir}/code/antifyFunctional.sh ants_ ${templateDir}/vold2_mni_head.nii.gz ${deconvName}+orig
fi
